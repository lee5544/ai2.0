...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="提取 mel 频谱特征并保存到 dl_dataset_csv")
    ...
    parser.add_argument("--output-file-prefix", default=None, help=f"特征 CSV 文件前缀（默认: {DEFAULT_OUTPUT_FILE_PREFIX}）")
    ...
    parser.add_argument("--n-mels", type=int, default=None, help="mel bin 数")
    parser.add_argument("--max-frames", type=int, default=None, help="最大时间帧数，超出截断，不足补零")
    parser.add_argument("--fmin", type=float, default=None, help="mel 最低频率")
    parser.add_argument("--fmax", type=float, default=None, help="mel 最高频率")
