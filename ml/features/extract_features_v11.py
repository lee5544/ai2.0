...
def extract_features_v11(
    ...
    *,
    ...
    timing["v11_chatter_sec"] = float(time.perf_counter() - t0)

    if return_timing:
        timing["total_sec"] = float(
            timing.get("total_sec", 0.0)
            + timing["v11_tick_sec"]
            + timing["v11_chatter_sec"]
        )
        return features, timing
    return features


extract_features = extract_features_v11
...
if __name__ == "__main__":
    ...
    feats, timing = extract_features_v11(demo, 20000, return_timing=True)
