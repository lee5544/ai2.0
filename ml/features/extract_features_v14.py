"""Feature extraction v14 (standalone).

v14 = v13 主体特征保留 + 鲁棒 PCEN 冲击增强。

PCEN 相对 v13 的改进：
  - 按物理时间常数平滑，不再依赖固定帧系数。
  - 全局稳健能量归一化，抑制传感器增益差异。
  - median + MAD/IQR 鲁棒冲击阈值和 prominence。
  - 按超阈值事件而非超阈值帧统计 shock_per_sec。
...
入口: extract_features_v14(data, sr, return_timing=False)
...
# Use a physical time constant in pcen_shock_features so that changing the
# sample rate or STFT hop does not change the smoother's temporal behaviour.
_PCEN_TIME_CONSTANT_SEC = 0.4
_PCEN_S = 0.025  # Fallback for standalone pcen_transform calls.
_PCEN_ALPHA = 0.98
_PCEN_DELTA = 2.0
_PCEN_R = 0.5
_PCEN_EPS = 1e-6
_PCEN_MAX_RELATIVE_ENERGY = 1e12
_PCEN_SHOCK_K_ROBUST = 4.0
_PCEN_WIN_SEC = 1.0
_PCEN_HOP_SEC = 0.5
_PCEN_PEAK_MIN_DISTANCE_SEC = 0.02
_PCEN_PEAK_PROMINENCE_K_ROBUST = 1.5
...
def _pcen_smoothing_coefficient(frame_sr: float, time_constant_sec: float) -> float:
    """Return the one-pole PCEN coefficient for a physical time constant."""
    t_frames = max(float(frame_sr) * float(time_constant_sec), _EPS)
    # Stable form of (sqrt(1 + 4*T**2) - 1) / (2*T**2).
    return float(2.0 / (np.sqrt(1.0 + 4.0 * t_frames * t_frames) + 1.0))


def _robust_location_scale(values: np.ndarray) -> tuple[float, float]:
    """Return median and a contamination-resistant Gaussian-equivalent scale."""
    v = np.asarray(values, dtype=np.float64).ravel()
    v = v[np.isfinite(v)]
    if v.size == 0:
        return 0.0, _EPS
    ...
    mad_scale = 1.4826 * float(np.median(np.abs(v - median)))
    if v.size >= 4:
    ...
    scale_floor = _EPS * max(abs(median), 1.0)
    return median, max(mad_scale, iqr_scale, scale_floor)
...
def pcen_transform(
    ...
    *,
    ...
    # Put eps and delta on a relative energy scale. A single global scalar
    # preserves all channel ratios while making PCEN insensitive to sensor
    # gain and recording amplitude over many orders of magnitude.
    positive = E[E > 0.0]
    if positive.size:
        energy_ref = max(float(np.median(positive)), np.finfo(np.float64).tiny)
        E = np.clip(E / energy_ref, 0.0, _PCEN_MAX_RELATIVE_ENERGY)

    s = float(np.clip(s, _EPS, 1.0))
    alpha = float(np.clip(alpha, 0.0, 1.0))
    delta = max(float(delta), 0.0)
    r = max(float(r), _EPS)
    eps = max(float(eps), _EPS)

    M = np.empty_like(E, dtype=np.float64)
    # Feature extraction is offline, so a short robust look-ahead can prevent
    # a first-frame shock from poisoning the smoother for several time constants.
...
def pcen_shock_features(
    ...
    对 low / mid / high / all 四个 mel 频带组分别提取 10 个特征：
      ...
      5. shock_per_sec:    envelope > median + k * robust_scale 的事件密度
    ...
    if power.ndim != 2 or power.shape[1] == 0:
        return {}
    if not np.isfinite(sr) or float(sr) <= 0.0:
        raise ValueError("sr must be a positive finite value")
    if int(hop_length) <= 0:
        raise ValueError("hop_length must be positive")

    frame_sr = float(sr) / float(hop_length)
    pcen_s = _pcen_smoothing_coefficient(frame_sr, _PCEN_TIME_CONSTANT_SEC)
    P = pcen_transform(power, s=pcen_s)
    ...
    for band_name, idx in bands.items():
        ...
        robust_center, robust_scale = _robust_location_scale(env)
        ...
        threshold = robust_center + _PCEN_SHOCK_K_ROBUST * robust_scale
        above = env > threshold
        # Count threshold-crossing episodes, not frames. One ringing impulse is
        # therefore one event and the rate is stable across different hops.
        shock_count = int(np.sum(above & ~np.r_[False, above[:-1]]))
        features[f"{base}_shock_per_sec"] = float(shock_count / max(duration, _EPS))

        # A max-based threshold lets one extreme artefact hide all real peaks.
        prominence = _PCEN_PEAK_PROMINENCE_K_ROBUST * robust_scale
...
def extract_features_v14(
    ...
    *,
    ...
    """v14 = v13 主体 + 鲁棒 PCEN 冲击增强；独立文件，不 import v13。
...
extract_features = extract_features_v14
...
if __name__ == "__main__":
    ...
    feats, timing = extract_features_v14(demo, 20000, return_timing=True)
