...
class OCSVMModel:
    ...
    - 分数解释：decision_function 越大越“正常”（inlier），故用其相反数作“异常分数”
    """
    ...
    def __init__(self, config_path: str):
        ...
        # 2) 构造 OCSVM 模型（建议加标准化）
        mcfg = cfg.get("model", {})
        ocsvm = OneClassSVM(
            kernel=mcfg.get("kernel", "rbf"),   # 'rbf' | 'linear' | 'poly' | 'sigmoid'
            nu=mcfg.get("nu", 0.05),            # 期望异常比例上界，典型 0.01~0.1
            gamma=mcfg.get("gamma", "scale"),   # 'scale' | 'auto' | float
            degree=mcfg.get("degree", 3),
            coef0=mcfg.get("coef0", 0.0),
            tol=mcfg.get("tol", 1e-3),
            shrinking=mcfg.get("shrinking", True),
            cache_size=mcfg.get("cache_size", 200)
        )
        self.model = Pipeline([
    ...
    def _load_and_sample_data(self) -> pd.DataFrame:
        ...
        for label, n in self.labeler.label_sample_dict.items():
            ...
            if n < 0:
                ...
                sampled = sub.copy()
                extra = sub.sample(n=need_extra, replace=True, random_state=42)
    ...
    def _split_data(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """与 XGB/IF 保持一致比例：test 10%，val 为剩余的 12.5%（即总 12.5%）"""
        try:
    ...
    def train_and_evaluate(self):
        ...
        # 训练集选择：仅 normal 或全部（OCSVM 推荐只用 normal）
    ...
    def train_and_cross_validate(self, n_splits=5):
        ...
        for fold, (tr_idx, te_idx) in enumerate(cv.split(X, y), 1):
            ...
            ocsvm = Pipeline([
                ...
                ("ocsvm", OneClassSVM(**self.model.named_steps["ocsvm"].get_params()))
            ])
            if self.fit_on == "normal_only":
                X_fit = X_tr[y_tr == 0]
            else:
            ...
            s_te = ocsvm.decision_function(X_te)
    ...
    def train_and_evaluate_grid(self):
        ...
        # 在 fit 时根据 y 过滤 normal（若配置为 normal_only）
        class OCSVMWrapper(Pipeline):
            def __init__(self, fit_on="normal_only", **oc_kwargs):
                steps = [
                    ("scaler", StandardScaler()),
                    ("ocsvm", OneClassSVM(**oc_kwargs))
                ]
        ...
        param_grid = {
            "ocsvm__kernel": ["rbf"],
            "ocsvm__nu": [0.01, 0.03, 0.05, 0.1],
            "ocsvm__gamma": ["scale", "auto", 0.1, 0.01, 0.001],
            # 如需 poly/sigmoid，可扩展：
            # "ocsvm__kernel": ["rbf","linear","poly","sigmoid"],
            # "ocsvm__degree": [2,3,4], "ocsvm__coef0": [0.0, 0.1, 1.0]
        }

        # 基于 decision_function 的 ROC AUC
        def roc_auc_est(est: OCSVMWrapper, Xv, yv):
            s = est.decision_function(Xv)  # 越大越正常
            return roc_auc_score(yv, -s)   # 取负号作为异常分数

        base = OCSVMWrapper(
            fit_on=self.fit_on,
            **self.model.named_steps["ocsvm"].get_params()
        )
        grid = GridSearchCV(
            base,
...
if __name__ == "__main__":
    ...
    # predictor:
    #   score_threshold: 0.0
    config = "epump4_ocsvm.yaml"   # 可直接复用你的配置文件
    model  = OCSVMModel(config)
